...
int main(int argc, char *argv[]) 
{
    ...
    max_fd = 18 //If this isn't set, then reader 1 doesn't work

    printf("Max fd %i\n", max_fd);

    FD_SET(receive_pipe_writer_0, &master);
    FD_SET(receive_pipe_writer_1, &master);
    FD_SET(receive_pipe_reader_0, &master);
    FD_SET(receive_pipe_reader_1, &master);

    while (1) 
    {    
        //wait for ask something
        reading = master;
